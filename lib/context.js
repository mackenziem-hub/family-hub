...
export async function buildAssistantContext(userId, userName) {
  ...
  const [groceries, nutrition, target, cal, kids, fitness, profile, photos, memories] = await Promise.all([
    ...
    recallMemories(userId, 30).catch(() => []),
  ]);
  ...
  // Long-term memory (durable facts the assistant has learned over time)
  if (memories && memories.length) {
    const ml = memories.slice(0, 30).map((m) => `(${m.category}${m.user_id ? '' : ', shared'})${m.pinned ? ' [pinned]' : ''} ${m.content}`);
    lines.push(`Long-term memory you've built about this family (treat as known, use it, keep it consistent):\n  - ${ml.join('\n  - ')}`);
  }
