...
export async function rememberFact(userId, { content, category, scope, pinned, dedup_key, source } = {}) {
  ...
  // Skip exact (case-insensitive) duplicates for the same owner. Escape LIKE metachars
  // (% _ \) so content like "20% off" or "swim_team" isn't treated as a wildcard, and
  // match the FULL content (not a 160-char slice, which would never match longer rows).
  const probe = content.replace(/[\\%_]/g, (c) => '\\' + c);
  const { data: dup } = await ownerFilter(supabaseAdmin.from('assistant_memory').select('id').ilike('content', probe)).limit(1);
