...
const CRISIS = /\b(kill myself|suicidal|suicide|end my life|want to die|wanna die|hurt myself|harm myself|self[-\s]?harm|cut(ting)? myself|no reason to live|better off dead|don'?t want to be alive)\b/i;

const DISORDERED = /(\bpurge\b|purging|make myself (throw up|vomit)|throw up (my|after)|laxative|crash diet|very low calorie|starve myself|starving myself|fast(ing)? (to|so i) lose|lose \d+\s?(lbs|pounds|kg)\s+(in|by)\s+(a|this|next|\d)\s*(day|week)|how (little|few) (can i eat|calories can i)|stop eating (to|so i) lose)/i;
